(function () {
  ...
  function count(show) {
    return P.countOf ? P.countOf(show) : (show.episodes || 0);
  }
  ...
  function render() {
    ...
    if (rail) {
      rail.innerHTML = P.shows.map(function (show) {
        return (
          ...
          '<em>' + esc(P.epLabel(count(show))) + '</em></a>'
        );
      }).join('');
    }
    ...
    if (latest) {
      latest.innerHTML = P.latestShows(4).map(function (show) {
        var ep = (P.latestEpisode && P.latestEpisode(show)) || show.latest || {};
        return (
    ...
    if (list) {
      list.innerHTML = P.shows.map(function (show) {
        return (
          ...
          '<small>' + esc(P.epLabel(count(show))) + '</small></span></a>' +
